...
const Lightfall: React.FC<LightfallProps> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Cleanup OGL resources - methods exist at runtime even if not typed
      const dispose = (obj: unknown): void => {
        if (!obj) return;
        const d = obj as { remove?: () => void; destroy?: () => void };
        d.remove?.();
        d.destroy?.();
      };
      dispose(programRef.current);
      dispose(geometryRef.current);
