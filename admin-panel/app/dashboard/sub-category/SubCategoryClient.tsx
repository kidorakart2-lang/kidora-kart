...
export default function SubCategoriesClient({
  ...
  const restoreMutation = useMutation({
    mutationFn: (id: string) => api.put(`/api/admin/subCategory/restore/${id}`, { id }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["subCategories"] });
      invalidateCache(["categories", "homepage", "navigation", "banners"]);
      toast({ title: "Sub category restored successfully" });
    },
    onError: (error: Error) => {
      toast({ title: error.message, variant: "destructive" });
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <AlertDialogUse
        ...
        description="Are you sure you want to soft-delete this sub category?"
