...
async function ensureIndexes(db) {
  ...
  const have = (keys) => idx.some((i) => JSON.stringify(i.key) === JSON.stringify(keys));
  console.log(`  ℹ️  slug unique: ${have({ slug: 1 }) ? "✅" : "❌ MISSING"}`);
  console.log(`  ℹ️  code unique (sparse): ${have({ code: 1 }) ? "✅" : "❌ MISSING"}`);
  console.log(`  ℹ️  sku unique (sparse): ${have({ sku: 1 }) ? "✅" : "❌ MISSING"}`);
  if (FIX) {
    await col.createIndex({ slug: 1 }, { unique: true });
    await col.createIndex({ code: 1 }, { unique: true, sparse: true });
    await col.createIndex({ sku: 1 }, { unique: true, sparse: true });
