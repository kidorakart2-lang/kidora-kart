    ...
    <div className="print-only bg-white" id="print-invoice">
        ...
        <div className="bg-indigo-50 border-l-4 border-indigo-500 p-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-indigo-900">{order.shipping.trackingNumber}</p>
            </div>
          </div>
          {order.shipping.carrier && <p className="text-xs text-indigo-700 mt-1">Courier: {order.shipping.carrier}</p>}
        </div>
      )}
        ...
        <div className="w-80">
          <div className="space-y-2 text-sm">
            ...
            {(order.pricing?.discount?.amount ?? 0) > 0 && (
              <div className="flex justify-between py-2"><span className="text-gray-600">Discount:</span><span className="font-semibold text-green-600">-₹{order.pricing?.discount?.amount?.toLocaleString("en-IN")}</span></div>
            )}
      ...
      <div className="border-t-2 border-gray-200 pt-6 mt-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-gray-600">Questions? Contact us at {process.env.NEXT_PUBLIC_SUPPORT_EMAIL || "support@kidorakart.com"}</p>
