            ...
            {allSubCategories.map((subCat) => (
              <SwiperSlide key={subCat._id}>
                <Link href={`/category/${subCat.parentSlug}/${subCat.slug}`}>
                  <article
                    className="group relative w-full aspect-square flex flex-col items-center cursor-pointer"
                    itemScope
                    itemType="https://schema.org/Thing"
                  >
                    <div
                      className="relative w-full h-full overflow-hidden flex items-center justify-center rounded-lg shadow-lg"
                      style={{
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent flex items-end justify-center">
                        <p
                          className="text-base sm:text-lg md:text-xl font-semibold text-white text-center px-2 pb-4 drop-shadow-lg"
                          itemProp="name"
                        >
                          {subCat.name}
                        </p>
