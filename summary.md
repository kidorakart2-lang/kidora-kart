...
> 🗂️ **Repository moved.** Production code now lives in a dedicated repo at
> `https://github.com/jewellerywalaonline-oss/jewellery-walla-monorepo.git` (remote name: `production`).
> The old `origin` (kidora-kart) is kept for history only. Branch workflow: `main` = stable
> production code, `test` = testing branch for new code (see `readme.md` → *Repository & Branch Workflow*).
